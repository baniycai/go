...
// 缓存池，还搞了两个，bufioWriter2kPool和bufioWriter4kPool，666
...
func (c *conn) readRequest(ctx context.Context) (w *response, err error) {
	...
	if d := c.server.ReadTimeout; d > 0 {
		wholeReqDeadline = t0.Add(d) // =ReadHeaderTimeout+ReadTimeout+now
	}
	...
	for k, vv := range req.Header {
		if !httpguts.ValidHeaderFieldName(k) { // 验证header的每一个key的每个字符是否都是ASCII字符
...
func (c *conn) serve(ctx context.Context) {
	...
	// NOTE 关键步骤，开始循环读取request
	for {
		w, err := c.readRequest(ctx) // 返回response
		if c.r.remain != c.server.initialReadLimitSize() {
			// If we read any bytes off the wire, we're active.
			c.setState(c.rwc, StateActive, runHooks)
		}
		// NOTE 其实很简单，就是按照http协议的格式人肉写回响应体
		if err != nil {
...
func (srv *Server) Serve(l net.Listener) error {
	...
	if !srv.trackListener(&l, true) { // 将l加入listeners map
		return ErrServerClosed
	}
	defer srv.trackListener(&l, false) // 删除l
	...
	for {
		...
		if err != nil {
			...
			if ne, ok := err.(net.Error); ok && ne.Temporary() { // 出现短暂的读错误时进行休眠后重试，最长休眠间隔为1s
				if tempDelay == 0 {
		...
		tempDelay = 0
		c := srv.newConn(rw) // 没啥，就是将srv和rw包一起得到conn
		// 追踪连接(conn),将其加入activeConnMap，并执行状态改变hook
		c.setState(c.rwc, StateNew, runHooks) // before Serve can return
		go c.serve(connCtx)                   // NOTE 新起一个协程来处理
	}
}
