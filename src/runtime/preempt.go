...
// Goroutine 抢占
//
//可以在任何安全点抢占 goroutine。目前，有几个类别的安全点：
//
//1. 阻塞的安全点发生在 goroutine 被取消调度、阻塞在同步操作上或在系统调用期间。
//
//2. 同步的安全点发生在运行中的 goroutine 检查是否存在抢占请求时。
//
//3. 异步的安全点发生在用户代码中任何可以安全暂停 goroutine 并且保守的堆栈和寄存器扫描可以找到堆栈根的指令处。
//   运行时可以使用信号在异步安全点停止一个 goroutine。
//
//在阻塞和同步安全点，goroutine 的 CPU 状态最小，并且垃圾回收器对其整个堆栈具有完整的信息。
//这使得可以以最小的空间来取消调度 goroutine，并精确地扫描 goroutine 的堆栈。
//
//同步安全点是通过在函数 prologue 中重载堆栈边界检查来实现的。为了在下一个同步安全点抢占 goroutine，
//运行时将 goroutine 的堆栈边界设为无效值，这样下一个堆栈边界检查就会失败并进入堆栈增长实现，
//该实现将检测到实际上是一个抢占并重定向到抢占处理。
//
//在异步安全点进行抢占是通过使用 OS 机制（例如信号）暂停线程并检查其状态以确定 goroutine 是否在异步
//安全点上。由于线程挂起本身通常是异步的，因此它还会检查正在运行的 goroutine 是否想要被抢占，
//因为这可能已经发生了更改。如果满足所有条件，它将调整信号上下文使其看起来像是触发的线程刚刚调用了 asyncPreempt，并恢复线程。
//asyncPreempt 溢出所有寄存器并进入调度程序。
//
//（另一种选择是在信号处理程序中进行抢占。这将让操作系统保存和恢复寄存器状态，而运行时只需要知道如何从信号上下文中提取可能包含指针的寄存器即可。
//但是，这将为每个被抢占的 G 都消耗一个 M，并且调度程序本身不适合从信号处理程序中运行，因为它倾向于在抢占路径中分配内存并启动线程。）
