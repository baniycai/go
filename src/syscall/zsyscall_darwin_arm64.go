...
// 在Golang中，uintptr(unsafe.Pointer(rsa))是将指针rsa转换为uintptr类型。
//该操作的作用是将指针转换为一个整数类型的值，以便在需要对指针进行一些底层的操作时使用。
//
//uintptr是Go语言的一种无符号整数类型，它的大小与当前系统的指针大小相同。
//unsafe.Pointer是Go语言中的一种特殊类型，它可以存储任何类型的指针，并且可以将这些指针强制转换为其他类型的指针。
//
//因此，在将指针rsa转换为uintptr类型之前，需要先将它转换为unsafe.Pointer类型。
//这个转换可以绕过Go语言的类型检查机制，因此需要小心使用，只有在必要的时候才应该使用它。

//go:cgo_import_dynamic libc_wait4 wait4 "/usr/lib/libSystem.B.dylib"

// THIS FILE IS GENERATED BY THE COMMAND AT THE TOP; DO NOT EDIT
// NOTE 最终还是通过汇编调用了C的方法
...
// 在zsyscall_darwin_arm64.s中(应该是)映射到C的对应方法，
...
// 这是一个特殊的编译指令，用于在 Go 语言中使用 CGo（C 和 Go 之间的互操作性）。
//具体来说，这个指令告诉 Go 编译器在运行时动态加载名为 "chroot" 的 C 函数，并将其绑定到 Go 中名为 "libc_chroot" 的函数上，
//该 C 函数实现了 chroot 系统调用功能。在 macOS 上，libSystem.B.dylib 是系统默认的 C 库，因此该指令将从该库中动态加载 chroot 函数。
//go:cgo_import_dynamic libc_chroot chroot "/usr/lib/libSystem.B.dylib"

// 注意：macOS 默认安装了 C 库（也称为标准 C 库或系统 C 库），它包含了许多常见的 C 语言函数和工具，例如 malloc、printf、scanf 等等。
//在 macOS 上，系统默认的 C 库是 libSystem.B.dylib，它包含了 POSIX 标准库以及其他一些 Mac 特定的功能。
//因此，如果您在编写 C 代码或使用 CGo 进行 C 和 Go 的互操作时，不需要手动安装 C 库，可以直接使用 macOS 自带的库
